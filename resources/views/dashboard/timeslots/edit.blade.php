...
<div class="container-fluid py-4">
    <div class="card shadow-lg border-0">
        <div class="card-header bg-theme text-white d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Edit Timeslot</h5>
            <a href="{{ route('admin.timeslots.index') }}" class="btn btn-light btn-sm">Back to Timeslots</a>
        </div>
        <div class="card-body p-4">
            <form method="POST" action="{{ route('admin.timeslots.update', $timeslot->id) }}" id="timeslotForm">
                @csrf
                @method('PUT')
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('doctor_id') is-invalid @enderror" id="doctor_id" name="doctor_id" required disabled>
                                ...
                                @foreach ($doctors as $doctor)
                                    <option value="{{ $doctor->id }}" {{ old('doctor_id', $timeslot->doctor_id) == $doctor->id ? 'selected' : '' }}>{{ $doctor->name }}</option>
                                @endforeach
                            </select>
                            @error('doctor_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <input type="hidden" name="doctor_id" value="{{ old('doctor_id', $timeslot->doctor_id) }}">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="mb-3">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="time" class="form-control @error('start_time') is-invalid @enderror" id="start_time" name="start_time" value="{{ old('start_time', \Carbon\Carbon::parse($timeslot->start_time)->format('H:i')) }}" required>
                            @error('start_time')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="time" class="form-control @error('end_time') is-invalid @enderror" id="end_time" name="end_time" value="{{ old('end_time', \Carbon\Carbon::parse($timeslot->end_time)->format('H:i')) }}" required>
                            @error('end_time')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-theme px-4"><i class="fas fa-save me-1"></i> Update Timeslot</button>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('Script loaded for timeslots/edit.blade.php');
